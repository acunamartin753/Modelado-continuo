...
using Plots, LinearAlgebra, LinearSolve

# ╔═╡ ccccfa81-2d50-4d72-ba31-fde564c6e60b
md""" # Ej 1 """

# ╔═╡ e5f9a938-7a32-448e-901b-68e8843d315f
function explicito(nh,r,alfa,contorno)
	h = ((1/(nh-1)))
	k = r * h * h / alfa
	m = Int(ceil(1/h+1))
	n = Int(ceil(1/k))
	...
	u = zeros(n,m)
	for j in 2:m-1
		u[1,j] = 10  # Falta poner las condiciones iniciales como una funcion
	end
	for i in 2:n
		u[i,1]=contorno[1]
		u[i,m]=contorno[2]
	...
	for i in 2:n
		for j in 2:m-1
        	u[i, j] = (u[i-1,j] + alfa*((u[i-1, j+1] - 2 * u[i-1, j] + u[i-1, j-1]) / h*h))
		end
	end
...
function implicito(nh,r,alfa,contorno)
	h = ((1/(nh-1)))
	k = r * h * h / alfa
	m = Int(ceil(1/h+1))
	n = Int(ceil(1/k))
	...
	A = Tridiagonal(fill(-1*alfa,nh-3),fill(2*alfa+1,nh-2),fill(-1*alfa,nh-3))
	
	for i in 2:n
		b = u[i-1,2:m-1]
		b[1] = b[1] + alfa*u[i,1]
		b[m-2] = b[m-2] + alfa*u[i,m]
		sol = A\b
		u[i,2:m-1]=sol
	end
	heatmap(u)
...
# ╔═╡ 3c2085c5-d3e0-456c-bc34-b418079058cd
explicito(100,0.5,0.5,[0,0])

# ╔═╡ 190b5c6e-5b0e-4ba3-998c-aefc6bbb73fb
implicito(100,0.5,0.9,[0,0])
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "3b5ccc437e639ea2193858161fb6fdb9275fe4c1"
