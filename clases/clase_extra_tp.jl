...
# ╔═╡ f08973e2-62aa-11ee-142d-0113604fcd31
using DifferentialEquations, Optimization, OptimizationOptimJL,Plots

# ╔═╡ 6db6a0ac-f77b-4e57-8e3f-d0dc5dc799fa
...
# ╔═╡ 6ae975fb-1323-49b5-b6b4-ff37ac7e2822
function create_data(p)
	prob = ODEProblem(predpresa!,[5,50],(0,500.),p)
	sol  = solve(prob)
	max.(sol(0:500.,idxs=1) + 3*randn(501),1.)
end

# ╔═╡ 00f1727b-f80f-4e7f-b26d-bfc1479f0976
data = create_data([0.075,0.0025,0.1,0.005])

# ╔═╡ ce9a99b7-3bd7-4a3e-aab1-16d6e1ff7ed1
scatter(data)

# ╔═╡ 69d8ec32-51cb-49b4-ba7c-13fd36cb9c71
function func(x,r)
	tot_loss = 0.0
	prob     = ODEProblem(predpresa!,x[1:2],(0,500.),x[3:6])
	sol      = solve(prob,Vern7())
    if any((!SciMLBase.successful_retcode(s.retcode) for s in sol))
        tot_loss = Inf
    else
		sol_eval = sol.(0:500.,idxs=1)
		tot_loss = sum((sol_eval - data).^2)
	end
	return tot_loss
end

# ╔═╡ 62e604b0-eda6-4c1e-a8b1-05eadbc830a6
optprob = OptimizationProblem(func,[5,50,0.01,0.01,0.01,0.01],lb=zeros(6),ub=[100,100,0.1,0.1,0.1,0.1])

# ╔═╡ 59af337a-017a-4e3a-996a-3422784bf3d2
p̄ = solve(optprob,SAMIN(),maxiters=100000)

# ╔═╡ 7cf9912c-6e34-43b8-bf8a-800383f1c506
begin
	prob_final = ODEProblem(predpresa!,p̄[1:2],(0,500),p̄[3:6])
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "18c5dd7a45d195c1b1a73192ade3fd6d89ffad91"
